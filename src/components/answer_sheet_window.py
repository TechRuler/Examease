...
from widget.auto_scrollbar import AutoScrollbar
import os,json
class AnswerSheet(Toplevel):
    def __init__(self,master,setting,exam_id,selected_answer,view_window,**kwarg):
        ...
        answer_sheet_window_scrollbar = AutoScrollbar(self,orient='vertical')
        ...
        answer_sheet_text.tag_configure("wrong",foreground=self.setting["exam-window"]["wrong_answer_color"])
        answer_sheet_text.tag_configure("right",foreground=self.setting["exam-window"]["correct_answer_color"])
        ...
        answer_sheet_text.grid(row=0,column=0,sticky='nsew')
        answer_sheet_window_scrollbar.grid(row=0,column=1,sticky='ns')
